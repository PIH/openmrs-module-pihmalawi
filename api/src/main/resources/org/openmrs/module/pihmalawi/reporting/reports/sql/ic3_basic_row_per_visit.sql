...
select encounter_type_id into @tbScreen from encounter_type where uuid = '745F221B9-7254-4B15-811B-5B8C8912F245'; -- 154
...
SELECT 
    ...
    (
    CASE
        WHEN vl.valueNumeric IS NOT NULL THEN vl.obsDate 
        WHEN lessvl.valueNumeric IS NOT NULl THEN lessvl.obsDate
        WHEN ldlvl.valueCoded IS NOT NULL THEN ldlvl.obsDate
	END)
    ) AS 'Viral load date',
    ...
    ), NULL )) 'Reason for no sample',
    vl.valueNumeric 'Hiv viral load',
    lessvl.valueNumeric 'Hiv Viral load less than limit',
    ...
    (select name from concept_name where voided = 0 and locale = "en" and concept_name_type = "FULLY_SPECIFIED" and concept_id = tb_test_sample.valueCoded) 'Sample received',
    ...
    " "))))) 'Rifampin Resistance',
    (select name from concept_name where voided = 0 and locale = "en" and concept_name_type = "FULLY_SPECIFIED" and concept_id = tb_test_noresultres.valueCoded) 'Reason for No Result',
...
JOIN 
	patient_identifier pi ON e.patient_id = pi.patient_id and pi.voided = 0
LEFT JOIN 
	person_address pa on pa.person_id = e.patient_id and pa.voided = 0
where v.date_stopped <= @endDate AND v.date_started >= @startDate 
