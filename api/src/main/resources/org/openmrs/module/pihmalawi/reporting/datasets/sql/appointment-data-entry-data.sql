...
(
    ...
    FROM
        encounter en
            JOIN
        location l ON l.location_id = en.location_id
            JOIN
        encounter_type ent ON ent.encounter_type_id = en.encounter_type
            JOIN
        patient_identifier pi ON pi.patient_id = en.patient_id
            JOIN
        patient_identifier_type pit ON pi.identifier_type = pit.patient_identifier_type_id
            JOIN
        person_name pn ON pn.person_id = en.patient_id
    WHERE
        DATE(en.encounter_datetime) = @endDate
            AND en.location_id = @location
            AND en.voided = 0
            AND pi.location_id = @location
            AND pit.name IN (@arv_number_name, @chronic_number_name)
            AND en.patient_id NOT IN (SELECT
                o.person_id
            FROM
                obs o
                    JOIN
                encounter en ON en.encounter_id = o.encounter_id
            WHERE
    ...
    FROM
        ...
        encounter en ON en.encounter_id = o.encounter_id
            JOIN
        location l ON l.location_id = o.location_id
            JOIN
        encounter_type ent ON ent.encounter_type_id = en.encounter_type
            JOIN
        patient_identifier pi ON pi.patient_id = en.patient_id
