...
CONCAT('Q', QUARTER(@startDate), '-', YEAR(@startDate)) AS quarter_label,
SUM(initial_visit_date BETWEEN @startDate AND @endDate) AS newly_total_registered_in_quarter,
SUM(initial_visit_date <= @endDate) AS cummulative_ever_total_registered,

SUM(IF(gender='M' AND transfer_in_date IS NULL AND initial_visit_date BETWEEN @startDate AND @endDate, 1, 0)) AS newly_ft_male,
SUM(IF(gender='M' AND transfer_in_date IS NULL AND initial_visit_date <= @endDate, 1, 0)) AS cummulative_ft_male,

SUM(IF(gender='F' AND transfer_in_date IS NULL AND (initial_pregnant_or_lactating != 'Patient pregnant' OR initial_pregnant_or_lactating IS NULL) AND initial_visit_date BETWEEN @startDate AND @endDate, 1, 0)) AS newly_ft_female_nonpreg,
SUM(IF(gender='F' AND transfer_in_date IS NULL AND (initial_pregnant_or_lactating != 'Patient pregnant' OR initial_pregnant_or_lactating IS NULL) AND initial_visit_date <= @endDate, 1, 0)) AS cummulative_ft_female_nonpreg,

SUM(IF(gender='F' AND transfer_in_date IS NULL AND initial_pregnant_or_lactating = 'Patient pregnant' AND initial_visit_date BETWEEN @startDate AND @endDate, 1, 0)) AS newly_ft_female_preg,
SUM(IF(gender='F' AND transfer_in_date IS NULL AND initial_pregnant_or_lactating = 'Patient pregnant' AND initial_visit_date <= @endDate, 1, 0)) AS cummulative_ft_female_preg, 

SUM(CASE WHEN transfer_in_date IS NULL AND initial_visit_date BETWEEN @startDate AND @endDate THEN 1 ELSE 0 END) AS total_ft,
SUM(CASE WHEN transfer_in_date IS NULL AND initial_visit_date <= @endDate THEN 1 ELSE 0 END) AS cummulative_ft,

#re-initiated

#transfer in patients
SUM(transfer_in_date BETWEEN @startDate AND @endDate) AS initial_transfer_In,
SUM(transfer_in_date <= @endDate) AS cummulative_transfer_In,
    
#all age at initiation
SUM(IF(initial_visit_date BETWEEN @startDate AND @endDate AND gender = 'M', 1, 0)) AS initial_males_all_ages,
SUM(IF(initial_visit_date BETWEEN @startDate AND @endDate AND gender = 'F' AND (initial_pregnant_or_lactating != 'Patient pregnant' OR initial_pregnant_or_lactating IS NULL), 1, 0)) AS initial_fnp_all_ages,
SUM(IF(initial_visit_date BETWEEN @startDate AND @endDate AND gender = 'F' AND initial_pregnant_or_lactating = 'Patient pregnant', 1, 0)) AS initial_fp_all_ages,
...
#children and adults at initiation
SUM(age < 24 AND initial_visit_date BETWEEN @startDate AND @endDate) AS age_at_initiation_less_24m,
SUM(age BETWEEN 24 AND 168 AND initial_visit_date BETWEEN @startDate AND @endDate) AS age_at_initiation__24m_14yrs,
SUM(age >= 180 AND initial_visit_date BETWEEN @startDate AND @endDate) AS age_at_initiation_15plus,
