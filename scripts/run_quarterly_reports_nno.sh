...
FILE=Quarterly_Cross_Site_Indicator_Upper_Neno-`echo $TODAY`.xls
run_report.sh \
  "PIH Quaterly Cross Site_" \
  "userEnteredParams%5BstartDate%5D=$THREE_MONTHS_AGO&userEnteredParams%5BendDate%5D=$NOW" \
...
# APZU HIV Indicators
FILE=APZU_HIV_Indicators_Quarterly_Upper_Neno-`echo $TODAY`.xls
run_report.sh \
  ...
  "" \
  $FILE
echo "" | mailx -a $FILE -s "emr: Upper Neno Quarterly APZU HIV Indicators $TODAY" "$MAIL"
mv $FILE /home/emradmin/pihmalawi/scripts/history
