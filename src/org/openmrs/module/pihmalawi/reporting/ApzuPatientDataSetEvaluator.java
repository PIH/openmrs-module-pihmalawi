...
public class ApzuPatientDataSetEvaluator implements DataSetEvaluator {
		...
		for (Patient p : patients) {
			...
			if (!encounters.isEmpty()) {
				...
				if (definition.getIncludeFirstVisit()) {
					c = new DataSetColumn("First visit", "First visit", String.class);
					e = encounters.get(0);
					row.addColumnValue(c, formatEncounterDate(e.getEncounterDatetime()));
				}
			...
			for (Relationship r : ships) {
				if (r.getRelationshipType().equals(
				    ...
				    Context.getPersonService().getRelationshipTypeByUuid("19124310-a89d-11df-bba5-000c297f1161"))) {
					vhw = r.getPersonB().getGivenName() + " " + r.getPersonB().getFamilyName() + " (Guardian)";
				} 
			}
	...
	private void chronicCare(Patient p, DataSetRow row) {
		...
		boolean deceased = (p.getDead() || p.getCauseOfDeath() != null || p.getDeathDate() != null);
		row.addColumnValue(c, (deceased ? "died" : "&nbsp;"));

		// cc diagnose
		String diag = "";
		obs = Context.getObsService().getObservationsByPersonAndConcept(p,
		    Context.getConceptService().getConceptByName("CHRONIC CARE DIAGNOSIS"));
		i = obs.iterator();
		while (i.hasNext()) {
