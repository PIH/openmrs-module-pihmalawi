...
/**
 * Custom form to add a patient... currently just a copy of the core ShortPatientFormController,
 * with the addition of a call to our MalawiPatientValidator to allow us to assign the IC3 ID if necessary
 */

@Controller
public class MalawiShortPatientFormController {

    private static final Log log = LogFactory.getLog(org.openmrs.web.controller.patient.ShortPatientFormController.class);

    private static final String SHORT_PATIENT_FORM_URL = "/module/pihmalawi/shortPatientForm";
    ...
    @Autowired
    MalawiPatientValidator malawiPatientValidator;
    ...
    public boolean getIdentifierLocationUsed() {
        ...
        for (PatientIdentifierType pit : pits) {
            if (pit.getLocationBehavior() == null || pit.getLocationBehavior() == PatientIdentifierType.LocationBehavior.REQUIRED) {
    ...
    public String saveShortPatient(WebRequest request, @ModelAttribute("personNameCache") PersonName personNameCache,
        ...
        if (Context.isAuthenticated()) {
         ...
         /*   new ShortPatientFormValidator().validate(patientModel, result);
            ...
            // custom validator for Malawi that adds in a IC3 screening number if needed
            malawiPatientValidator.validate(patient, patientErrors);
